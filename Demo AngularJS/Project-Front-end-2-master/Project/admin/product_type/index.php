...
{
	header("location: http://localhost:80/project/login/index.php"); 
}
else{
?>
<!DOCTYPE html>
					...
					<div class="widget-content nopadding">
						...
						<form class="form-horizontal">
							<div class="control-group">
								...
								<label class="control-label">ProductType Name :</label>
								<div class="controls">
									<input type="text" name="name" ng-model="name" class="span11">
								</div>
							</div>
							<div class="form-actions">
								<input type="submit" value="{{NameNut}}" ng-click="insertData()" class="btn btn-success"></input>
							</div>
						...
						<table class="table table-bordered table-striped">
							...
							<tbody ng-init="displayData()">
								<tr ng-repeat="protype in protypes | filter:timkiem | orderBy: sortColumn:reverse | limitTo:limitRow">
									...
									<td>{{protype.name}}</td>
									<td>
										<button ng-click="updateData(protype.id, protype.name)" class="btn btn-success btn-mini">Update</button> 
